...
class MultiModelService(BentoService):
    ...
    def boundary_detection(self, imgs):
        response_log = dict()
        ...
        if use_full_img:
            ...
            id_image_arr = img_arr[
                boundary_box[1] : boundary_box[3], boundary_box[0] : boundary_box[2], :
            ]
        ...
        return [{"id_image_arr": id_image_arr, "boundary_angle": boundary_angle}]

    @api(input=JsonInput(), batch=False)
    def kv_detection(self, data):
        id_image_arr, boundary_angle = (
            np.array(data["id_image_arr"], dtype="uint8"),
            data["boundary_angle"],
        )
        ...
        if settings.ID_FORCE_TYPE:
            kv_boxes, kv_scores, kv_classes = filter_class(
        ...
        return [
            {
                ...
                "id_image_arr": id_image_arr,
                "kv_boxes": kv_boxes,
                "kv_scores": kv_scores,
                "kv_classes": kv_classes,
            }
        ]
    ...
    def recognition(self, data):
        ...
        print("\033[95m" + f"{cropped_images.shape}" + "\033[m")
        print("\033[95m" + f"{np.array(cropped_images[0]).shape}" + "\033[m")
        print("\033[95m" + f"{np.array(cropped_images[1]).shape}" + "\033[m")
        class_masks = data["class_masks"]
    ...
    def _boundary_infer(self, img_arr):
        ...
        if use_mask:
            try:
                boxes, labels, scores, masks = self.artifacts.boundary_detection.run(
                    output_names, inputs
                )
            except Fail:
            ...
            try:
                (
                    ...
                    scores,
                    keypoints,
                ) = self.artifacts.boundary_detection.run(output_names, inputs)
            except Exception as e:
