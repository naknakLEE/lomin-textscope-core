...
class DocumentModelService(BentoService):
    ...
    def recognition_preprocessing(self, cropped_images):
        ...
        cropped_images = torch.stack(cropped_images).to("cuda")
    ...
    def detection_result_visualization(self, img, detection_boxes, detection_classes):
        self.savepath = (
            f"{settings.BASE_PATH}/others/assets/deidentify_img_sk_{self.detection_threshold}.jpg"
        )
    ...
    def document_ocr(self, img):
        ...
        img_tensor = self.detection_preprocessing(img.copy())
        with torch.no_grad():
        ...
        # np.save(
        #     f"{settings.BASE_PATH}/assets/document_ocr_results",
        #     {
        #         "rec_preds": rec_preds,
