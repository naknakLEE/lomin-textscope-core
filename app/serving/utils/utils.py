...
default_to_regular = lambda d: {k: default_to_regular(v) for k, v in d.items()} if isinstance(d, defaultdict) else d
...
def build_resize(resize_cfg):
    ...
    if resize_mode == 'fixed':
        ...
        def resize_func(img_arr, extra_info=None):
            return cv2.resize(img_arr, (target_width, target_height), interpolation=cv2.INTER_LINEAR), extra_info
        ...
        def resize_func(img_arr, min_size=min_size, max_size=max_size, extra_info=None):
            ...
            return cv2.resize(img_arr, (ow, oh), interpolation=cv2.INTER_LINEAR), extra_info

    elif resize_mode == 'keep_ratio+0.5':
        target_width = resize_cfg['width']
        target_height = resize_cfg['height']
        def resize_func(img_arr, target_width=target_width, target_height=target_height, extra_info=None):
            h, w = img_arr.shape[0:2]
        ...
        def resize_func(img_arr, extra_info=None, ow=ow, oh=oh):
            ...
            if (1./cut_ratio_th < img_aspect_ratio) and (cut_ratio_th > img_aspect_ratio):
                #'keep_ratio+min0.5'
                if img_aspect_ratio > tgt_aspect_ratio:
                    img_target_size = (ow, max(int((h *(ow/w))),  oh//2))
                else:
                ...
                blank[:img_target_size[1], :img_target_size[0],:] = cv2.resize(img_arr, img_target_size, interpolation=cv2.INTER_LINEAR)
                return blank, extra_info

            if img_aspect_ratio < 1.0:
                cut_ratio_list = [1/(cut_ratio_th*4),1/(cut_ratio_th*3),1/(cut_ratio_th*2),1/cut_ratio_th]
                ...
                for idx in range(cut_size):
                    ...
                    min_x = 0; max_x = w
                    min_y = crop_y0; max_y = _cut_h
                    cropped_img = img_arr[min_y:max_y, min_x:max_x, :]

                    min_x = paste_x0; max_x = paste_x0 + cropped_img.shape[1]
                    min_y = 0; max_y = cropped_img.shape[0]
                ...
                cut_ratio_list = [cut_ratio_th*4,cut_ratio_th*3,cut_ratio_th*2,cut_ratio_th]
                cut_size_list = [5,4,3,2]
                cut_size = 2
                for idx, ratio_th in enumerate(cut_ratio_list):
                    if img_aspect_ratio > ratio_th:
                ...
                paste_y0 = 0
                for idx in range(cut_size):
                    min_x = crop_x0; max_x = min(crop_x0 + cut_length, w)
                    min_y = 0; max_y = h
                    cropped_img = img_arr[min_y:max_y, min_x:max_x, :]

                    min_x = 0; max_x = cropped_img.shape[1]
                    min_y = paste_y0; max_y = paste_y0 + cropped_img.shape[0]
                    blank[min_y:max_y,min_x:max_x,:] = cropped_img
                    crop_x0 += cut_length
                    paste_y0 += h
                img_arr = blank
...
def mask_to_quad(mask ,box, mask_threshold=0.5, force_rect=False, resize_ratio=5.0):
    ...
    if resize_ratio > 1.0:
        mask_pred = cv2.resize(mask_pred, fx=resize_ratio, fy=resize_ratio, dsize=None, interpolation=cv2.INTER_CUBIC)
...
def save_debug_img(img_arr, kv_boxes, kv_classes, texts, savepath):
    ...
    for _box, _class, _text in zip(kv_boxes, kv_classes ,texts):
        ...
        cv2.putText(_img_arr, f'{_class}: {_text}', (min_x, min_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,0,0), 2)
