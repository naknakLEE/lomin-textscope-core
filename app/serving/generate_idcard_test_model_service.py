...
# traced_net = onnx.load(f"{settings.BASE_PATH}/assets/models/gocr_chshin_efficientdet_d4_210607_model.ts")
traced_net = torch.jit.load(f"{settings.BASE_PATH}/assets/models/resnet-18.pt").to(
    torch.device("cuda")
)
# traced_net = torch.jit.load(f"{settings.BASE_PATH}/assets/models/mask_rcnn.pt").to(torch.device("cuda"))
