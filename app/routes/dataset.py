...
def insert_data(
    ...
    for category in track(new_categories, description="Inserting category and image..."):
        ...
        category_image_paths = list(filter(lambda x: category.name in x.as_posix(), image_paths))
        for image_path in category_image_paths:
            query.insert_image(
                session=session,
...
def get_cls_train_dataset(
    ...
    categories = query.select_category_all(session, dataset_pkey=dataset.dataset_pkey)
