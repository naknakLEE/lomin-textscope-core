...
async def remove_data_job():
    ...
    if(len(result)): 
        minio_client.move_file_to_local_directory(bucket_name=settings.MINIO_IMAGE_BUCKET, document_path_list=result, local_directory=hydra_cfg.document.no_update_directory)

    # 3. Minio 삭제
    minio_client.remove_(bucket_name=settings.MINIO_IMAGE_BUCKET, object_name=delete_date)
