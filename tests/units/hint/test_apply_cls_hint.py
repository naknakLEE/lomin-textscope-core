...
class TestApplyClsHint:
    ...
    def test_trust_true_and_exist_cls_result(self):
    ...
    def test_trust_true_and_not_exist_cls_result(self):
        doc_type_hint = DocTypeHint(use=True, trust=True, doc_type=self.hint_doc_type)
    ...
    def test_trust_false_and_reliable_cls_result(self):
    ...
    def test_trust_false_and_unreliable_cls_result(self):
        doc_type_hint = DocTypeHint(use=True, trust=False, doc_type=self.hint_doc_type)
    ...
    def test_trust_false_and_not_exist_cls_result(self):
        doc_type_hint = DocTypeHint(use=True, trust=False, doc_type=self.hint_doc_type)
        output = apply_cls_hint(doc_type_hint)
        assert output == self.expected_form

    def test_use_false(self):
        doc_type_hint = DocTypeHint(use=True, trust=False, doc_type=self.hint_doc_type)
        output = apply_cls_hint(doc_type_hint)
        assert output == self.expected_form

    def test_doc_type_is_none(self):
