...
class TestUser:
    ...
    def test_create_user(self, get_db: Session):
        user = Users.create(
            get_db, email=self.email, hashed_password=self.hashed_password
        )
        if user:
    ...
    def test_authenticate_user(self, get_db: Session):
        user = Users.create(
            get_db, email=self.email, hashed_password=self.hashed_password
        )
    ...
    def test_not_authenticate_user(self, get_db: Session):
        user = Users.authenticate(get_db, email=self.email, password=self.password)
        assert user is None

    def test_check_if_user_is_active(self, get_db: Session):
        user = Users.create(
            get_db, email=self.email, hashed_password=self.hashed_password
        )
        if user:
    ...
    def test_check_if_user_is_active_inactive(self, get_db: Session):
        user = Users.create(
            get_db,
            email=self.email,
    ...
    def test_check_if_user_is_superuser_normal_user(self, get_db: Session):
        user = Users.create(
            get_db, email=self.email, hashed_password=self.hashed_password
        )
        if user:
    ...
    def test_get_user(self, get_db: Session):
        user = Users.create(
            get_db,
            email=self.email,
            hashed_password=self.hashed_password,
    ...
    def test_update_user(self, get_db: Session):
        user = Users.create(
            get_db,
            email=self.email,
            hashed_password=self.hashed_password,
