...
def obfuscate_variable(
    ...
    *replacement*. *right_of_equal*, and *inside_parens* are used to determine
    ...
    if len(re.findall("app", token_string)) >= 1 and prev_tok_string == "from":
        return "__skipline__"

    # if len(token_string.split('.')) > 1:
    #     token_in_token = token_string.split(".")
    #     for token in token_in_token:
            

    # if len(re.findall(r"{(.*?)\}", token_string)) >= 1:
    #     token = re.findall(r"{(.*?)\}", token_string)[0]
    #     token = token.split(".")[0]
    #     token_in_token = re.findall(r"\((.*?)\)", token)
    #     if len(token_in_token) > 0:
    #         token = token_in_token[0]

    #     key = next((k for k in VAR_REPLACEMENTS if VAR_REPLACEMENTS[k] == token), None)
    #     if key is not None and len(re.findall('f"', key)) == 0:
    #         token_string = re.sub(token, key, token_string)
    #     return return_replacement(token_string)
    # if len(re.findall('f"', token_string)) == 1 or len(re.findall(r"{(.*?)\}", token_string)) >= 1:
    #     if len(re.findall('f"{serving_server_inference_url}/{request_api}"', token_string)):
    #         ic(token_string)
    #     included_token = re.findall(r"{(.*?)\}", token_string)
    #     for token in included_token:
    #         token = token.split(".")[0]
    #         token_in_token = re.findall(r"\((.*?)\)", token)
    #         if len(token_in_token) > 0:
    #             token = token_in_token[0]
...
def obfuscate_builtins(module, tokens, name_generator, table=None):
    ...
    for builtin in used_builtins:
        replace_obfuscatables(module, tokens, obfuscate_unique, builtin, iter_replacements)
...
def obfuscate(module, tokens, options, name_generator=None, table=None):
    ...
    *module* must be the name of the module we're currently obfuscating
    ...
    if options.obfuscate:
        variables = find_obfuscatables(tokens, obfuscatable_variable, ignore_length=ignore_length)
        classes = find_obfuscatables(tokens, obfuscatable_class)
        functions = find_obfuscatables(tokens, obfuscatable_function)
        for variable in variables:
