...
def test_function():
    ...
    foo = (
        "The # character in this string should " "not result in a syntax error"  # This comment
    )  # ...and this one should go away
...
def pyminify(options, files):
    ...
    if any(obfuscations):
        ...
        if options.use_nonlatin:
            if sys.version_info[0] == 3:
                ...
                print("ERROR: You can't use nonlatin characters without Python 3")
                sys.exit(2)
        else:
    ...
    for sourcefile in files:
        ...
        list_of_tokens_to_add = list()
        prev_token_string = None
        line = -1
        for i, token in enumerate(copy_tokens):
            if prev_token_string == "#":
                ic(prev_token_string)
                ic(token)
            if line == token[2][0]:
                continue
            else:
                prev_token_string = token[1]
                line = -1
            if len(re.findall("#", prev_token_string)) > 0:
                line = token[2][0]
                continue
            prev_token_string = token[1]
            
            # string_tokens = re.split('/|f"', token[4])
            # re.split(r'{(.*?)\}|/|f"|"',  example)
            # ic(token[1], token[4])
            # len(re.findall(token[1], token[4])) < 1
            # if len(re.findall('f"', token[4])) < 1:
            included_tokens = re.findall(r"{(.*?)\}", token[1])
            if len(included_tokens) < 1:
                # if len(re.findall('f"', token[4])) < 1:
                list_of_tokens_to_add.append(token)
                continue
            
            # if len(included_tokens) < 1  or len(re.findall('f"', token[1])) < 1:
            #     # if len(re.findall('f"', token[1])) < 1:
            #     continue
            string_ = ""
            for i, character in enumerate(token[1]):
                if character == "{" or  character == "}" or character == "." or character == "=":
                    start_position = i + token[2][1]
                    end_position = start_position + len(string_) - 1
                    created_string_set = [
                            1,
                            string_,
                            (token[2][0], start_position),
                            (token[3][0], end_position),
                            token[4],
                    ]
                    list_of_tokens_to_add.append(created_string_set)
                    string_ = ""

                    start_position = end_position
                    end_position = end_position + 1

                    created_character_set = [
                            53,
                            character,
                            (token[2][0], start_position),
                            (token[3][0], end_position),
                    ...
                    list_of_tokens_to_add.append(created_character_set)
                    continue
                string_ += character
                if len(token[1]) - 1 == i:
                    start_position = i + token[2][1]
            ...
            # ic(token)
            # for included_token in included_tokens:
            #     token_in_token = re.findall(r"\((.*?)\)", included_token)
            #     if len(token_in_token) > 0:
            #         included_token = token_in_token[0]
            #     sepatated_by_dot_token_list = included_token.split(".")
            #     for each_token in sepatated_by_dot_token_list:
            #         start_position = token[1].strip().find(each_token) + token[2][1]
            #         end_position = start_position + len(each_token) - 1
            ...
            #             token[4],
            #         ]
            #         list_of_tokens_to_add.append(created_token_set)
            #         ic(created_token_set)
            # if len(included_tokens) > 1:
            #     del tokens[i]
        # tokens.extend(list_of_tokens_to_add)
        tokens = list_of_tokens_to_add
        # ic(tokens)

        # if len(string_tokens) <= 1:
        #     continue
        # for string_token in string_tokens:
        #     start_position = token.find(string_token) + token[2][1]
        #     end position = start_position + len(string_token) + token[3][1]
        # created_token_set = [77, string_token,
        #     (token[2][0], start_position),
        #     (token[2][0], end_position),
        #     token]
        # list_of_tokens_to_add.append(created_token_set)
        # for token in tokens:
        #     ic(token)
        # Perform obfuscation if any of the related options were set
        ic(os.path.split(sourcefile)[1])
        if name_generator and os.path.split(sourcefile)[1] != "const.py":
            obfuscate.obfuscate(
        ...
        result += "# Created by pyminifier " "(https://github.com/liftoff/pyminifier)\n"
        ...
        filepath = os.path.split(sourcefile)[0]
        # ic(filepath)
        saved_directory_path = os.path.join(options.destdir, filepath)
        os.makedirs(saved_directory_path, exist_ok=True)
        path = os.path.join(options.destdir, sourcefile)
        # ic(path)
        ...
        print(
            (
                ...
                "({percent_saved}% of original size)"
            ).format(**locals())
        )
