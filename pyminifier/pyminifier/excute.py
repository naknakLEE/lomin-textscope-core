...
def set_token_value(token_type: int, token: Dict, string_: str, start_position: int, end_position: int) -> List:
    created_string_set = [
            token_type,
            string_,
    ...
    return created_string_set


def split_token(token: Dict, position_start: Optional[tuple] = None) -> List:
    list_of_tokens_to_add = list()
    string_ = ""
    for i, character in enumerate(token[1]):
        # 특수문자 세트 가져와서 사용하는게 좋을지
        special_characters = '"!@#$%^&*()-+?=.,<>/"{} '
        if any(c in special_characters for c in character):
        # if character == "{" or  character == "}" or character == "." or character == "=" or character == "," or character == " ":
            if position_start is not None:
                start_position = position_start[1]
            else:
                start_position = token[2][1]
            end_position = start_position + len(string_) - 1
            created_string_set = set_token_value(token_type=1, token=token, string_=string_, start_position=start_position, end_position=end_position)
            list_of_tokens_to_add.append(created_string_set)

            start_position = end_position
            end_position = end_position + 1
            created_character_set = set_token_value(token_type=1, token=token, string_=character, start_position=start_position, end_position=end_position)
...
def pyminify(options, files):
    ...
    if len(files) > 1: # We're dealing with more than one file
        ...
        for sourcefile in files:
            ...
            prev_token_string = ""
            line = -1
            for _, token in enumerate(copy_tokens):
                if line == token[2][0]:
                    continue
                else:
                ...
                if len(included_tokens) < 1:
                    ...
                    continue
                list_of_tokens_to_add.extend(split_token(token))

            tokens = list_of_tokens_to_add

            # Perform obfuscation if any of the related options were set
            if name_generator and os.path.split(sourcefile)[1] != "const.py" and os.path.split(sourcefile)[1] != "models.py":
                var_replacements = obfuscate.obfuscate(
                    module,
                    tokens,
                    options,
            ...
            if original:
                ...
                sourcefile = sourcefile[1:] if sourcefile[0] == "/" else sourcefile
