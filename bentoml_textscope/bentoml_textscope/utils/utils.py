...
from lovit.preprocess import augmentation_impl as T
from lovit.preprocess import aggamoto
from lovit.utils.converter import CharacterMaskGenerator, build_converter


from bentoml_textscope.utils.envs import logger, settings
from bentoml_textscope.utils.catalogs import ELabelCatalog, EDocumentCatalog
from bentoml_textscope.errors.exceptions import InferenceException
...
def load_models(infer_sess_map: dict, service_cfg: dict, orb_matcher: dict = dict()):
    ...
    for resource in resources:
        ...
        # _model_path = os.path.join(settings.BASE_PATH, settings.INFERENCE_SERVER_APP_NAME, resource['model_path']) if 'model_path' in resource else ''
...
def find_rotated_angle(original_image):
    transforms = [
        T.ResizeShortestEdge(
            [settings.ROTATE_IMAGE_MIN_SIZE, settings.ROTATE_IMAGE_MIN_SIZE],
            settings.ROTATE_IMAGE_MAX_SIZE,
        )
    ]
    resized_image = aggamoto.preprocess_image(original_image, transforms)
    blur_image = cv2.medianBlur(resized_image, settings.MEDIANBLUR_PATCH_SIZE)
