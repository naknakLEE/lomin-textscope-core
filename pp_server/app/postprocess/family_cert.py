...
from maskrcnn_benchmark.utils.post_process import merge_boxes
from maskrcnn_benchmark.data.datasets.postprocess.commons import _get_iou_x, _get_iou_y, BoxlistPostprocessor as PP
...
def get_bottom_boundary(keyword_map, bottom_keywords):
    ...
    for keyword in bottom_keywords:
        if keyword in keyword_map and \
            len(keyword_map[keyword]) > 0:
...
def get_issue_date(pred, keyword_map, bottom_keywords):
    ...
    for keyword in DATE_KEYWORDS:
        ...
        for i, bbox in enumerate(target_boxlist.bbox):
            if (closest_bbox_idx is None or \
                bbox[1] < target_boxlist.bbox[closest_bbox_idx][1]):
                closest_bbox_idx = i
...
def get_title(pred):
    ...
    if len(split_title) < 2:
        ...
        return f"{split_title[0]}(상세)" if "상" in split_title[1] \
            else f"{split_title[0]}(특정)" if "특" in split_title[1] \
            else f"{split_title[0]}(일반)" if "일" in split_title[1] \
            else title

def find_values(boundary_bbox, pred, debug_dic, keyword, idx, max_y=None, x_iou_thres=0.25, is_name=False):
    bboxes = pred.bbox

    x_iou_scores = _get_iou_x(np.expand_dims(boundary_bbox, axis=0), pred.bbox, divide_by_area2=True)
    x_iou_mask = x_iou_scores > x_iou_thres
    x_iou_mask = torch.squeeze(x_iou_mask, dim=0)

    t_mask = bboxes[:, 1] > boundary_bbox[3]
    b_mask = bboxes[:, 3] < max_y if max_y is not None else torch.ones((bboxes.size(0),), dtype=torch.bool)
    ...
    if len(values) > 0:
        debug_dic.update({'%s_%d_%d'%(keyword, idx[0], idx[1]) : [{'bbox' : value[0], 'text' : value[1]} for value in values]})
    ...
    return values + find_values(boundary_bbox, pred, debug_dic, keyword, (idx[0], idx[1] + 1), max_y, x_iou_thres, is_name)
    ...
    for keyword, padding in zip(PERSONAL_INFO_KEYWORDS, PADDING_FACTOR):
        ...
        for i in range(len(keyword_map[keyword])):
            ...
            if keyword == '성명':
                ...
                bvs = find_values(boundary_bbox, pred, debug_dic=debug_dic, keyword=keyword, idx=(i, 0), max_y=max_y, x_iou_thres=0.99, is_name=True)
            else:
                width = target[2] - target[0]
                boundary_bbox = np.array([target[0] - width / 2 * padding, target[1], target[2] + width / 2 * padding, target[3]])
                bvs = find_values(boundary_bbox, pred, debug_dic=debug_dic, keyword=keyword, idx=(i, 0), max_y=max_y)
    ...
    for idx_base, base_bv in enumerate(base_bvs):
        ...
        for keyword in PERSONAL_INFO_KEYWORDS[1:]:
            ...
            if target_mask.sum() > 0:
                ...
                if y_iou_score.max() < 0.05:
                    ...
                    for i in range(10):
                        ...
                        base_bbox[1] = ymin
                        base_bbox[3] = ymax
                        y_iou_score = _get_iou_y(base_bbox[None, :], target_bboxes, divide_by_area2=True)
                        if y_iou_score.max() > 0.05:
                            flag = True
                            break
                    if flag:
            ...
            if keyword == "성명":
                ...
                is_dead = (len(dead_checker) > 1 and len(dead_checker[-1]) > 1) \
                    or (len(value) > 2 and "사망" in value)
                value = value.split("(")[0]
...
def validate_parent(kbv_dict):
    ...
    if len(set(['부', '모']) & set([_[1] for _ in gubun_bvs])) > 0:
        ...
        if len(gubun_bvs) > 2 and \
            ((gubun_bvs[1][1] != "부" and gubun_bvs[2][1] == "모") or
        ...
        if '본인' in all_gubun_keys and '배우자' in all_gubun_keys:
            ...
            gubun_father_bbox = [interpolate(a, b, ip_father) for a, b in zip(gubun_self_bbox, gubun_spouse_bbox)]
            gubun_mother_bbox = [interpolate(a, b, ip_mother) for a, b in zip(gubun_self_bbox, gubun_spouse_bbox)]
