...
from maskrcnn_benchmark.data.datasets.postprocess.commons import _remove_invalid_characters, _remove_others_in_text
from maskrcnn_benchmark.data.datasets.postprocess.family_cert import get_personal_info, get_issue_date, obj_to_kvdict, bbox_hcmp, bbox_vcmp
...
def postprocess_basic_cert(pred, score_thresh=0.5, *args):
    ...
    try:
        personal_info, personal_info_debug = get_personal_info(filtered_pred, keyword_map, BOTTOM_KEYWORDS)
    except:
        personal_info = []
        personal_info_debug = {}
