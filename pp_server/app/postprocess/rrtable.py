...
from .commons import BoxlistPostprocessor as PP
import torch
...
def get_doc_type(pred):
    ...
    indices = [i for i, text in enumerate(texts) if any(np.array([jamo_levenshtein(key, text) for key in ['병역사항', '병역', '병사역항', '처분일자', '처분사항']]) < 0.5)]
    if len(indices) == 0:
        return 'copy', {}
...
def get_abstract_user_info(pred):
    ...
    texts = pred.get_field('texts')
    key_pred = pred[np.array([True if re.search("성명|주민등록번호|한자", text) else False for text in texts])]
    debug_dict.update({'regnum_name_key' : key_pred})
    if len(key_pred) > 0:
        filter_pred = abs(pred.bbox[:,1]-key_pred.bbox[np.argmin(key_pred.bbox[:,1])][1]) < (key_pred.bbox[np.argmin(key_pred.bbox[:,1])][3]-key_pred.bbox[np.argmin(key_pred.bbox[:,1])][1])/2
        ...
        for idx, text in enumerate(user_info_pred.get_field('texts')):
            ...
            elif any(k in text for k in ['(', ')', '한자']) and len(kv_dict['name']) == 0 and len(user_info_pred) > idx + 1:
...
def get_copy_user_info(pred):
    ...
    if len(key_pred) > 0:
        ...
        filtered_pred = pred[pred.bbox[:,0] - (key_pred.bbox[0][0] - (key_pred.bbox[0][2] - key_pred.bbox[0][0]) / 2) > 0]
        filtered_pred = filtered_pred[filtered_pred.bbox[:,2] - (key_pred.bbox[0][2] + (key_pred.bbox[0][2] - key_pred.bbox[0][0]) / 2) < 0]

        texts = filtered_pred.get_field('texts')
        regnum_pred = filtered_pred[np.array([True if re.search("\d{6}", text) else False for text in texts])]
        ...
        for regnum_bbox, regnum_text in zip(regnum_pred.bbox, regnum_pred.get_field('texts')):
            ...
            name_mask = (pred.bbox[:, 3] < regnum_bbox[1] + regnum_height / 2) & (pred.bbox[:, 2] > regnum_bbox[0] - regnum_width / 2) \
            ...
            if len(name_candidates) > 0:
                ...
                name_filter[np.argmin(abs(name_candidates.bbox[:,3]-regnum_bbox[1]) + abs(name_candidates.bbox[:,0]-regnum_bbox[0]))] = True
            ...
            category_mask = (pred.bbox[:, 0] < name_bbox[0]) & (pred.bbox[:, 0] < regnum_bbox[0]) \
                & (pred.bbox[:, 1] > name_bbox[1] - name_height / 2) & (pred.bbox[:, 3] < regnum_bbox[3] + regnum_height / 2) \
                if len(name_candidates) > 0 else np.array([False] * len(category_mask))

            category_candidates = pred[category_mask]
            if len(category_candidates) > 0:
                category_filter = np.array([False] * len(category_candidates))
                category_filter[np.argmin(abs(category_candidates.bbox[:, 2] - name_bbox[0]))] = True
...
def get_issue_date(pred):
    ...
    if len(key_pred) > 0:
        ...
        for idx in range(len(key_pred)):
            ...
            filter_pred = (abs(pred.bbox[:,1]-key_pred.bbox[idx][1]) < (key_pred.bbox[idx][3]-key_pred.bbox[idx][1])/2) & (pred.bbox[:, 0] > left_boundary)
            date_pred = pred[torch.tensor(filter_pred, dtype=torch.bool)]
            if all(any([key in text for text in date_pred.get_field('texts')]) for key in ['년','월','일']):
                find_date = True
                break
        if find_date:
            if len(date_pred) > 1:
...
def get_army_info(pred):
    ...
    filter_pred = abs(pred.bbox[:,1]-key_pred.bbox[-1][1]) < (key_pred.bbox[-1][3]-key_pred.bbox[-1][1])/2
